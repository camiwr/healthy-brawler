...
export class LevelThreeScene extends Scene {
    ...
    create() {
        // --- 1. CARREGAR MAPA E TILESETS DO NÍVEL 3 ---
        this.map = this.make.tilemap({ key: 'map_level3' });

        // Nomes dos tilesets (do Tiled e do Preloader)
        const tileset1 = this.map.addTilesetImage('tileset', 'tileset_terrain');
        const tileset2 = this.map.addTilesetImage('tileset_att', 'tileset_att_objects');
        const tileset3 = this.map.addTilesetImage('boat', 'boat');

        let wallsLayer;

        // --- 2. CRIAR LAYERS (Com base no level3.tmx) ---
        // (Assumindo quais tilesets criam quais camadas)
        if (tileset1) {
            this.map.createLayer('water', tileset1);
            this.map.createLayer('sand', tileset1);
            wallsLayer = this.map.createLayer('wall', tileset1); 
        }

        if (tileset2 && tileset3) {
            // Usar ambos os tilesets nas layers que precisam deles
            this.map.createLayer('objects', [tileset2, tileset3]);
            this.map.createLayer('bridge', [tileset2, tileset3]);
        } else {
            if (tileset2) {
            this.map.createLayer('objects', tileset2);
            this.map.createLayer('bridge', tileset2);
            }
            if (tileset3) {
            this.map.createLayer('bridge', tileset3);
            }
        }
        // Definir colisão para a camada 'walls'
        if (wallsLayer) {
            wallsLayer.setCollisionByProperty({ collider: true });
        } else {
            console.error("Camada de colisão 'walls' não pôde ser criada.");
        }
    ...
    private winLevel(): void {
        if (this.scene.isActive()) { 
            console.log('Fase 3 Vencida!');
            
            this.scene.stop('UIScene');
            GameProgress.unlockNextLevel(3);
